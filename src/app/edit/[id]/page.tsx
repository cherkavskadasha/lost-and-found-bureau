...
import { Search, MapPin, FileText, Loader2, Building2, Save } from "lucide-react";
...
export default function EditItemPage() {
  ...
  useEffect(() => {
    const fetchItem = async () => {
      try {
        ...
        const catRes = await fetch('/api/seed'); 
  ...
  if (loading) {
    return <div className="py-20 flex justify-center"><Loader2 className="w-10 h-10 animate-spin text-indigo-500" /></div>;
  }
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-3xl animate-in fade-in duration-500">
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden shadow-indigo-100/20 p-8 space-y-6">
        
        <div className="space-y-4">
          <div className="space-y-1.5">
            <label className="text-sm font-semibold text-slate-700">Категорія (потрібно обрати заново)</label>
            <select 
          ...
          <div className="space-y-1.5">
            ...
            <input 
              type="text" 
              required
              className="w-full h-12 px-4 rounded-xl border border-slate-200 text-sm focus:ring-2 focus:ring-indigo-500 outline-none font-medium"
          ...
          <div className="space-y-1.5">
            ...
            <textarea 
              required
              rows={4}
              className="w-full p-4 rounded-xl border border-slate-200 text-sm focus:ring-2 focus:ring-indigo-500 outline-none resize-none"
        ...
        <div className="pt-4 flex items-center justify-end gap-4 border-t border-slate-100">
          ...
          <Button 
            type="submit"
            disabled={saving}
            className="bg-indigo-600 hover:bg-indigo-700 text-white rounded-full px-8 h-12 font-semibold shadow-md transition-all hover:scale-[1.02]"
          >
